...
# --- Initialize SQLite ---
conn = sqlite3.connect("child_health.db", check_same_thread=False)
c = conn.cursor()
...
with tab1:
    ...
    if st.button("Delete Patient Record"):
        confirm = st.checkbox("Confirm deletion? This will remove ALL data for this patient.", key="confirm_delete")
        if confirm:
            c.execute("DELETE FROM medical_history WHERE app_number=?", (st.session_state.app_number,))
            c.execute("DELETE FROM vaccinations WHERE app_number=?", (st.session_state.app_number,))
            c.execute("DELETE FROM child_details WHERE app_number=?", (st.session_state.app_number,))
            conn.commit()
            st.success(f"All data for application number {st.session_state.app_number} deleted!")
            st.session_state.app_number = str(uuid.uuid4())[:8]  # Reset application number
            st.experimental_rerun()
